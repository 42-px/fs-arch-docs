...
- фильтрация и ее настройки, которые в итоге собираются в параметры эффекта получения данных.
...
Фабрики обычно располагаются в отдельных директориях:
...
Ниже приведен пример фабрики-модели для создания отдельного окна верификации действия пользователя в системе через телефонный номер и присланный код:
    ...
    return (
      <>
        ...
        {agreementStep === AgreementStep.phone && (
          <ButtonWrapper>
            <Button onClick={() => validateAgreements()}>Отправить код</Button>
          </ButtonWrapper>
        )}
